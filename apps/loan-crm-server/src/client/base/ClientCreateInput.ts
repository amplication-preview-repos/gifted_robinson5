/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
} from "class-validator";
import { ApplicationCreateNestedManyWithoutClientsInput } from "./ApplicationCreateNestedManyWithoutClientsInput";
import { Type } from "class-transformer";
import { LoanCreateNestedManyWithoutClientsInput } from "./LoanCreateNestedManyWithoutClientsInput";

@InputType()
class ClientCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  address?: string | null;

  @ApiProperty({
    required: false,
    type: () => ApplicationCreateNestedManyWithoutClientsInput,
  })
  @ValidateNested()
  @Type(() => ApplicationCreateNestedManyWithoutClientsInput)
  @IsOptional()
  @Field(() => ApplicationCreateNestedManyWithoutClientsInput, {
    nullable: true,
  })
  applications?: ApplicationCreateNestedManyWithoutClientsInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  email?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  firstName?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  lastName?: string | null;

  @ApiProperty({
    required: false,
    type: () => LoanCreateNestedManyWithoutClientsInput,
  })
  @ValidateNested()
  @Type(() => LoanCreateNestedManyWithoutClientsInput)
  @IsOptional()
  @Field(() => LoanCreateNestedManyWithoutClientsInput, {
    nullable: true,
  })
  loans?: LoanCreateNestedManyWithoutClientsInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  phoneNumber?: string | null;
}

export { ClientCreateInput as ClientCreateInput };
